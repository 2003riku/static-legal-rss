...
class StaticLegalScraper:
    ...
    def __init__(self):
        ...
        self.site_configs = {
            'bengo4': {
                ...
                'list_url': 'https://www.bengo4.com/topics/',
                'selectors': {
                    'article_links': 'a[href*="/c_"]',
                    'title': 'h1',
                    'content': '.articleBody, .topicsBody',
                    'date': 'time, .date'
                }
            },
            'corporate_legal': {
    ...
    def get_article_links(self, site_key: str, max_links: int = 10) -> List[str]:
        """記事リンクを取得"""
    ...
    def extract_article_content(self, url: str, site_key: str) -> Dict:
        ...
        try:
            ...
            content = content_elem.get_text(strip=True)[:500] if content_elem else "内容不明"
    ...
    def scrape_site(self, site_key: str, max_articles: int = 10) -> List[Dict]:
        """特定サイトから記事を取得"""
        config = self.site_configs[site_key]
    ...
    def scrape_all_sites(self, max_articles_per_site: int = 10) -> List[Dict]:
        """全サイトから記事を取得"""
        all_articles = []
    ...
    def save_articles_json(self, articles: List[Dict], filepath: str):
        """記事データをJSONファイルに保存"""
...
if __name__ == "__main__":
    ...
    for article in articles[:3]:  # 最初の3件を表示
        print(f"\nタイトル: {article['title']}")
